package com.bw.mothons.view.fragment;


import android.content.Context;
import android.text.TextUtils;
import android.view.View;
import android.widget.Button;
import android.widget.EditText;
import android.widget.Toast;

import androidx.fragment.app.Fragment;

import com.bw.mothons.R;
import com.bw.mothons.base.BaseFragment;
import com.bw.mothons.contract.RegistContract;
import com.bw.mothons.model.bean.RegistBean;
import com.bw.mothons.presenter.RegistPresenter;

import java.util.regex.Pattern;

import butterknife.BindView;
import butterknife.OnClick;

/**
 * A simple {@link Fragment} subclass.
 */
public class RegistFragment extends BaseFragment<RegistPresenter> implements RegistContract.IView {


    @BindView(R.id.regist_phone)
    EditText registPhone;
    @BindView(R.id.regist_pwd)
    EditText registPwd;
    @BindView(R.id.regist)
    Button regist;


    @Override
    protected void initView(View inflate) {

    }

    @Override
    protected RegistPresenter providePresenter() {
        return new RegistPresenter();
    }

    @Override
    protected int layoutId() {
        return R.layout.fragment_shouye;
    }

    @Override
    protected void initData() {

    }

    @Override
    public void startActivitys(Context context, Class cls) {

    }

    @Override
    protected void onFragmentDestroy() {

    }

    @Override
    public void onRegistSuccess(RegistBean registBean) {
        Toast.makeText(getContext(), ""+registBean.getMessage(), Toast.LENGTH_SHORT).show();

    }

    @Override
    public void onRegistFailure(Throwable throwable) {

    }

    @OnClick(R.id.regist)
    public void onViewClicked() {
        String phones = registPhone.getText().toString().trim();
        if (TextUtils.isEmpty(phones)) {
            Toast.makeText(getContext(), "手机号不能为空", Toast.LENGTH_SHORT).show();
            return;
        }
        String pwds = registPwd.getText().toString().trim();

        if (TextUtils.isEmpty(pwds)) {
            Toast.makeText(getContext(), "密码不能为空", Toast.LENGTH_SHORT).show();
            return;
        }
      String  PHONES="^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\\d{8}$";
        boolean matches = Pattern.matches(PHONES, phones);
        if(matches){
            mpresenter.getRegistData(phones, pwds);
        }else{
            Toast.makeText(getContext(), "失败", Toast.LENGTH_SHORT).show();
        }

    }
}
